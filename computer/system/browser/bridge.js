...
import { DEVICE_ID, BROWSER_BRIDGE_PORT } from '../env.js';
...
function start() {
    ...
    wss.on('connection', (socket, req) => {
        // token 校验：?token=<DEVICE_ID>，挡掉本机其它进程乱连
        let token = '';
        try { token = new URL(req.url, 'http://127.0.0.1').searchParams.get('token') || ''; } catch {}
        if (DEVICE_ID && token !== DEVICE_ID) { try { socket.close(4001, 'bad token'); } catch {} return; }
    ...
    console.log(`🧩 浏览器 CDP 桥就绪：ws://127.0.0.1:${PORT}/cdp?token=${DEVICE_ID}`);
