...
export const useMemoriesStore = defineStore('memories', () => {
    ...
    function bind() {
        if (bound) return;
        bound = true;
        ws.onMessage('memories.list.result', onResult);
        ws.onMessage('memories.delete.result', onResult);
    }

    async function load() {
        bind();
        loading.value = true;
        const d = await request('memories.list');
        loading.value = false;
        if (d?.ok) items.value = d.items || [];
    }
    async function remove(id) {
        bind();
        const d = await request('memories.delete', { id });
        if (d?.ok) await load();
        return d;
    }
