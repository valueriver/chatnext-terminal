...
const canSend = computed(() => ws.connected && !chat.busy && !uploading.value && (input.value.trim().length > 0 || atts.value.length > 0));
...
function loadShortcuts() { if (ws.connected) shortcuts.load(); }
onMounted(loadShortcuts);
watch(() => ws.connected, (v) => { if (v) loadShortcuts(); });
    ...
    <div class="composer">
        ...
        <div class="inputwrap">
            ...
            <textarea
                ...
                v-model="input"
                rows="1"
                :placeholder="ws.connected ? '发消息，回车发送，Shift+回车换行' : '等待连接…'"
                :disabled="!ws.connected"
                @keydown="onKeydown"
                @input="autoGrow"
            ></textarea>
