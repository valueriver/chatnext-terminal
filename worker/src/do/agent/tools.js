...
export const tools = [
    // ───────── 云端原生(碰 D1,无需设备) ─────────
    {
        ...
        function: {
            ...
            description: [
                ...
                '表:notes / outlines / chats / messages / compactions / shortcuts / devices / settings。',
    ...
    // ───────── shell ─────────
    {
        ...
        function: {
            name: 'shell',
            description: '在目标设备上执行 shell 命令并返回输出(stdout+stderr)。用于查看/操作文件、运行程序、查询系统状态等。',
            parameters: {
                type: 'object',
                properties: {
                    device: DEVICE,
    ...
    // ───────── 浏览器:唯一工具,直发 CDP(经 browser-use 扩展驱动目标设备的 Chrome) ─────────
    {
        type: 'function',
        function: {
            name: 'browser_cdp',
            description: [
                '通过 Chrome DevTools Protocol 操作目标设备的 Chrome(经 browser-use 扩展)。这是唯一的浏览器工具:你直接发原始 CDP 方法,默认作用于当前活动标签页。',
                '常用:',
    ...
    },
    {
        ...
        function: {
            name: 'computer_key',
            description: '在目标设备按一个键,可带修饰键。例如 {key:"c",modifiers:["command"]} = ⌘C;{key:"enter"} = 回车。',
            parameters: {
                type: 'object',
                properties: {
                    device: DEVICE,
    ...
    },
    {
        ...
        function: {
            name: 'computer_click',
            description: '在目标设备鼠标点击(需 cliclick)。给 x,y 则移动到该逻辑坐标后点击;不给则在当前位置点击。',
            parameters: {
                type: 'object',
                properties: {
                    device: DEVICE,
    ...
    },
    {
        ...
        function: {
            ...
            description: '在目标设备当前位置滚动(需 cliclick)。',
            parameters: {
                type: 'object',
                properties: {
                    device: DEVICE,
